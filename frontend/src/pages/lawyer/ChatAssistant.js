...
import {
  ...
  BookOpen, Shield, FileText, Briefcase, RotateCcw, Gavel
} from 'lucide-react';
...
export default function ChatAssistant() {
  ...
  const suggested = [
    { icon: Gavel, text: 'Latest amendments in CrPC', color: 'from-blue-500 to-cyan-500' },
    { icon: FileText, text: 'Draft a bail application template', color: 'from-violet-500 to-purple-500' },
    { icon: Scale, text: 'Explain Section 138 NI Act', color: 'from-amber-500 to-orange-500' },
    { icon: Briefcase, text: 'Key points for corporate litigation', color: 'from-emerald-500 to-teal-500' },
    { icon: Shield, text: 'Rights of accused under Article 22', color: 'from-rose-500 to-pink-500' },
    { icon: BookOpen, text: 'Summarize recent Supreme Court ruling', color: 'from-indigo-500 to-blue-500' },
  ];
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="bg-white border-b border-slate-200 px-4 md:px-8 py-4 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-800">AI Legal Research Assistant</h1>
              <p className="text-xs text-slate-500">Powered by AI — Research cases, laws & drafting</p>
            </div>
          </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 md:px-8 py-6">
          {showWelcome ? (
            <div className="max-w-2xl mx-auto flex flex-col items-center justify-center h-full">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-xl shadow-emerald-200 mb-6">
                <Sparkles className="w-10 h-10 text-white" />
              </div>
              <h2 className="text-2xl font-bold text-slate-800 mb-2">How can I assist your practice?</h2>
              <p className="text-slate-500 text-sm mb-8 text-center max-w-md">Research case law, get statute summaries, draft legal documents, and explore recent judgments — all powered by AI.</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-lg">
                {suggested.map((s, i) => (
                  <button key={i} onClick={() => sendQuery(s.text)} className="group flex items-start gap-3 p-3.5 bg-white rounded-xl border border-slate-200 hover:border-emerald-300 hover:shadow-md hover:shadow-emerald-100 transition-all text-left">
            ...
            <div className="max-w-3xl mx-auto space-y-5">
              {messages.map(m => (
                <div key={m.id} className={`flex gap-3 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {m.role === 'assistant' && (
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center flex-shrink-0 mt-0.5 shadow-sm">
                      <Bot className="w-4 h-4 text-white" />
                    </div>
                  )}
                  <div className={`max-w-[75%] ${m.role === 'user'
                    ? 'bg-gradient-to-br from-emerald-600 to-teal-600 text-white rounded-2xl rounded-tr-md px-4 py-3 shadow-md shadow-emerald-200'
                    ...
                    {m.pending ? (
                      <div className="flex items-center gap-2 py-1">
                        ...
                        <span className="text-xs text-slate-400 ml-1">Researching...</span>
                      </div>
                    ) : (
                      <>
          ...
          <form onSubmit={handleSubmit} className="max-w-3xl mx-auto flex items-center gap-3">
            <div className="flex-1 relative">
              <input
                ...
                placeholder="Research a case, statute, or legal concept..."
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 pr-12 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent disabled:opacity-50 transition-all"
              />
              {loading && (
            ...
            <button
              ...
              className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-600 to-teal-600 text-white flex items-center justify-center shadow-lg shadow-emerald-200 hover:shadow-xl hover:shadow-emerald-300 disabled:opacity-40 disabled:shadow-none disabled:cursor-not-allowed transition-all"
            >
              <Send className="w-4 h-4" />
            </button>
          </form>
          <p className="text-center text-[10px] text-slate-400 mt-2">AI responses are informational only and should be verified with official sources.</p>
        </div>
      </div>
    </div>
  );
