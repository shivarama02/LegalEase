...
import {
  ...
  CheckCheck, Trash2, Filter, RefreshCw, Inbox, AlertCircle, Briefcase
} from 'lucide-react';

const TYPE_CONFIG = {
  appointment: { icon: Calendar, label: 'Appointment', color: 'from-blue-500 to-cyan-500', bg: 'bg-blue-50', text: 'text-blue-700' },
  feedback:    { icon: Star, label: 'Review', color: 'from-amber-500 to-orange-500', bg: 'bg-amber-50', text: 'text-amber-700' },
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-8 py-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {stats.unread > 0 ? <span className="text-emerald-600 font-medium">{stats.unread} unread</span> : 'All caught up!'} &middot; {stats.total} total
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              {stats.unread > 0 && (
                <button onClick={markAllRead} className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-colors">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            {[
              ...
              { label: 'Unread', value: stats.unread, icon: AlertCircle, color: 'from-emerald-500 to-teal-600' },
          ...
          <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1">
            ...
            {['all', 'appointment', 'feedback', 'chat', 'system', 'verification', 'reminder'].map(f => (
              <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 text-xs font-medium rounded-full whitespace-nowrap transition-colors ${filter === f ? 'bg-emerald-600 text-white shadow-sm' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50'}`}>
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {notifications.map(n => {
                ...
                const Icon = cfg.icon;
                return (
                  <div key={n.id} className={`group bg-white rounded-xl border shadow-sm p-4 flex items-start gap-4 transition-all hover:shadow-md ${n.is_read ? 'border-slate-200' : 'border-emerald-200 bg-emerald-50/30'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                          {!n.is_read && (
                            <button onClick={() => markRead(n.id)} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Mark read">
